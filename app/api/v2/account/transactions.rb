      ...
      class Transactions < Grape::API
        ...
        params do
          ...
          optional :deposit_state,
                   values: { value: ->(v) { Array.wrap(v).all? { |value| value.in? Deposit::STATES.map(&:to_s) } }, message: 'account.transactions.invalid_deposit_state' },
                   desc: 'Filter deposits by states.',
                   default: []

          optional :withdraw_state,
                   values: { value: ->(v) { Array.wrap(v).all? { |value| value.in? Withdraw::STATES.map(&:to_s) } }, message: 'account.transactions.invalid_withdraw_state' },
